function mergeScript(source,target) {
  ...
  ON   T.${dataform.projectConfig.vars.target_hash_unique_col_name}  =  S.hash_src_unique
      ...
      AND T.${dataform.projectConfig.vars.start_at_column_name} IS NULL  
  WHEN NOT MATCHED BY SOURCE AND t.${dataform.projectConfig.vars.start_at_column_name} is null THEN
  UPDATE 
  SET t.${dataform.projectConfig.vars.start_at_column_name} = 
          case 
              when t.${dataform.projectConfig.vars.start_at_column_name} is NULL then
          (select max(${dataform.projectConfig.vars.timestampfield}) from ${(source)})
              else t.${dataform.projectConfig.vars.start_at_column_name}
          end   
